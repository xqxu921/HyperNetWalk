...
methods_pers <- list(
  ...
  Merged_Plus = "./results/Merged_Plus/",
...
ggplot(df_comb_num, aes(x = n_gene, y = prec_nums, color = method)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  scale_color_manual(values = c("HyperNetWalk" = "#D94841", "Forward" = "#1F77B4", "Merged" = "#2CA02C", "Merged_Plus" = "#FF7F0E", "Undirected" = "#9467BD")) +
...
# Cohort results
# Cohort methods: HyperNetWalk, Forward, Merged, Merged_Plus, Undirected
