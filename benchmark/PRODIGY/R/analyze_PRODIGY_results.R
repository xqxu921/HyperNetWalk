...
analyze_PRODIGY_results<-function(all_patients_scores)
{
	...
	for(i in 1:length(all_patients_scores))
	{
		...
		} else if (nrow(all_patients_scores[[i]])==1) {
			sorted_idx = order(all_patients_scores[[i]],decreasing=T)
			ranking = all_patients_scores[[i]][,sorted_idx]
		...
		#filter pathways
			...
			if(length(pathways_to_take) < 2)
			{
				...
				if(all(ranking==0))
				{
					...
					ranking = sort(apply(all_patients_scores[[i]][pathways_to_take,,drop=F],2,function(x) sum(sort(x,decreasing=T))),decreasing=T)
				}
			} else {
