			...
			num_of_cores=1,sample_origins = NULL, write_results = F, results_folder = "./",beta=2,gamma=0.05,delta=0.05,mutation_list = NULL)
{
	...
	# num_cores <- min(100, parallel::detectCores() - 1)
	...
	# all_patients_scores <- foreach(i=1:length(samples), .packages = c("DESeq2","igraph","ff","plyr","biomaRt","parallel","PCSF","graphite"),.export = c('PRODIGY','get_pathway_list_from_graphite','get_diff_expressed_genes','get_enriched_pathways','get_pathway_network','get_pathway_genes_from_network','get_enrichment_pvalue','run_single_PCSF'),.options.snow=opts) %dopar% {
	# 	sample = samples[i]
	...
	# 	}
	# 	PRODIGY(sample_mutations,expression_matrix,network,sample,diff_genes,alpha=alpha,pathway_list=pathway_list,
	...
	# close(pb)
	# stopCluster(cl)
	# names(all_patients_scores) = samples
