...
def compute_influence_for_sample(sampleIdx, mutMatrix, diffGenes, Pathway, ppi_dict):
    ...
    mutGenes = sorted(mutMatrix.index[mutMatrix.iloc[:, sampleIdx] == 1].tolist()) 
    #这里加sorted是因为源代码用Matlab实现，而Matlab取交集后自动按字典顺序排序，因此在matlab结果中矩阵按照行名和列名的字典顺序排序；因为下述对overlapMatrix的错误定义导致结果和mutGenes的顺序有关..离谱
    ...
    overlapMatrix = np.zeros((numGenes, numGenes))
    for i in range(numGenes):
        for j in range(numGenes):
            ...
            e_i = hyperedges[i]
            e_j = hyperedges[j]
            inter = max(0.1, len(set(e_i).intersection(e_j)))
            min_len = min(len(e_i), len(e_j))
            overlapMatrix[i, j] = inter / min_len

    Infl = np.zeros(numGenes)
    for i, g_i in enumerate(mutGenes):
        for j, g_j in enumerate(mutGenes):
        # for j, g_j in enumerate(allGenes):
            ...
            Infl[i] = Infl[i] + overlapMatrix[i, j] * w_ij
...
def run_driver_prioritization(cancer):
    ...
    # result_eg = compute_influence_for_sample(args_list[297])
...
if __name__ == '__main__':
    ...
    results_dir = Path(__file__).resolve().parent.parent.parent/"results/PITCH"/cancer
