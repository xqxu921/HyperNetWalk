...
main <- function(cancer_types, base_dir = "./data") {
  ...
  raw_data_dir <- file.path(base_dir, "rawdata")
  if (!dir.exists(raw_data_dir)) {
    stop("Raw data directory not found: ", raw_data_dir)
  }
  
  gene_metadata_file <- file.path(base_dir, "gencode.v36.annotation.gtf.gene.probemap")
  ...
  for (cancer_type in cancer_types) {
    ...
    output_dir <- file.path(base_dir, "processed_data", cancer_type)
...
for (cancer_type in cancers) {
  # cancer_type <- "BRCA"
  mut_data_file <- paste0("./data/rawdata/TCGA-",
                          cancer_type,
                          ".somaticmutation_wxs.tsv")
  # mut_data_file <- paste0("./data/",
  #                         cancer_type,
  #                         "/",
  #                         cancer_type,
  #                         "_mc3_gene_level.txt")
  exp_data_file <- paste0('./data/rawdata/TCGA-',
                          cancer_type,
                          '.star_counts.tsv')
  gene_metadata_file <- "./data/gencode.v36.annotation.gtf.gene.probemap"
  output_dir <- paste0("./data/processed_data/", cancer_type, "/")
  if (!dir.exists(output_dir)) {
    dir.create(output_dir, recursive = T)
  }
  mut_data <- process_mut_data(mut_data_file)
  write.table(mut_data, file=paste0(output_dir, "mut_data.tsv"),sep="\t",quote=F,row.names=T,col.names=T)
  exp_data <- process_exp_data(exp_data_file, gene_metadata_file)
  write.table(exp_data, file=paste0(output_dir, "exp_data.tsv"),sep="\t",quote=F,row.names=T,col.names=T)
  # common_genes <- intersect(rownames(mut_data), rownames(exp_data))
  # common_samples <- intersect(colnames(mut_data), colnames(exp_data))
  # cat("For ", cancer_type, ":\n")
  # cat(
  #   "Number of common samples: ",
  #   length(common_samples),
